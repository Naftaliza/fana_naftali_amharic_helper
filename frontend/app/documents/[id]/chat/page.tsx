        ...
        {messages.map((m) => (
          <div key={m.id} className={`flex ${m.role === 0 ? "justify-end" : "justify-start"}`}>
            <div className={`max-w-[80%] whitespace-pre-wrap rounded-2xl px-4 py-2 ${
              m.role === 0 ? "bg-brand text-white" : "bg-gray-100 text-gray-800"
            }`}>
              {m.content}
            </div>
          </div>
        ))}
        {busy && <p className="text-sm text-gray-400">{t("common.loading")}</p>}
        <div ref={bottomRef} />
      </div>

      <form onSubmit={send} className="mt-3 flex gap-2">
        <Input value={question} onChange={(e) => setQuestion(e.target.value)} placeholder={t("chat.placeholder")} />
